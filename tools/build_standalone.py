...
# -*- coding: utf-8 -*-
"""
為每個 skill 產生 dist/STANDALONE/<skill>.md：SKILL.md（去除 YAML frontmatter）＋ references/ ＋ assets/。

    python tools/build_standalone.py            # 產生／更新全部單檔版
    python tools/build_standalone.py --check    # 只檢查是否為最新（CI 用，不寫檔）

**輸出刻意放在 skill 目錄外。** skill 目錄只放執行期材料——單檔版是給另一個發佈通道用的複本，
留在 skill 裡會被帶走 skill 的人一併拿到，也可能被 agent 誤開（它的檔名聽起來像「完整版」）。

單檔版供 ChatGPT Knowledge / Claude Project / Gemini Gems 上傳使用，內容必須與各分檔一致。
改動任何分檔後執行本腳本重新產生，不要手動編輯產出檔。
三個 skill 各有一份，需要跨 skill 的內容時成對上傳；只上傳其中一份時，另一份的參考檔會缺席。
"""
import argparse
import sys
...
>
> **路徑對照**：本檔已包含本 skill 全部 `references/`{assets_note} 的 Markdown 內容。內文提到
...
def main():
    ...
    ap.add_argument("--check", action="store_true",
                    help="只檢查 dist/STANDALONE/ 是否為最新，不寫檔")
    args = ap.parse_args()
    ...
    for skill_dir in sorted(p for p in SKILLS_DIR.iterdir() if (p / "SKILL.md").exists()):
        ...
        out = DIST_DIR / f"{skill_dir.name}.md"
